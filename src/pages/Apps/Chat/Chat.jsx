    ...
    <Box className="ChatConversation" height="100%" overflow="auto">
      ...
      >
        {(message) => {
          ...
          return (
            <Box key={`chat-conversation-${message?.id}`} direction="row" gap="small" pad="small" flex={false}>
              <Box>
...
function ChatConversations({ conversations, user, conversationId, selectConversation }) {
  return (
    <Box className="ChatConversations">
      {conversations.map((conversation) => {
        return (
          <Button key={conversation.id} onClick={() => selectConversation(conversation.id)} plain>
            <Box
